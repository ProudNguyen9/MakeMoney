param(
  [string]$BlogPath = 'blog.html',
  [string]$MainCssPath = 'assets/css/main.css'
)
...
$blog = Read-Text $BlogPath
$mainCss = Read-Text $MainCssPath
$legacyCss = $null

$styleMatch = [regex]::Match($blog, '(?s)\r?\n\s*<style>\s*(.*?)\s*</style>')
if ($styleMatch.Success) {
  $legacyCss = $styleMatch.Groups[1].Value.Trim()
  $blog = [regex]::Replace($blog, '(?s)\r?\n\s*<style>\s*.*?\s*</style>', '', 1)
}
...
$blog = [regex]::Replace(
  $blog,
  '<link href="https://fonts\.googleapis\.com/css\?family=Open\+Sans:300,300i,400,400i,600,600i,700,700i\|Roboto:300,400,500,700&display=swap" rel="stylesheet">',
  $newFontLink.Trim(),
  1
)

if ($blog -notmatch '<body[^>]*class="[^"]*\bblog-page\b') {
  if ($blog -match '<body[^>]*class="([^"]*)"') {
    $blog = [regex]::Replace($blog, '<body([^>]*)class="([^"]*)"([^>]*)>', '<body$1class="$2 blog-page"$3>', 1)
  }
  else {
    $blog = [regex]::Replace($blog, '<body\s*>', '<body class="blog-page">', 1)
  }
}
...
$heroBlock = @'
  <section class="hero scrap-hero blog-hero d-flex align-items-center">
...
$blog = [regex]::Replace($blog, '(?s)<header id="header" class="header">.*?</header>', $headerBlock.Trim(), 1)
$blog = [regex]::Replace($blog, '(?s)<footer id="footer" class="footer">.*?</footer>', $footerBlock.Trim(), 1)
$blog = [regex]::Replace(
  $blog,
  '(?s)<div class="social-button is-open">.*?</div>\s*(<a href="#" class="scroll-top[^>]*>)',
  { param($m) $socialButtonBlock.Trim() + "`r`n`r`n  " + $m.Groups[1].Value },
  1
)

if ($blog -notmatch 'class="hero scrap-hero blog-hero') {
  $mainReplacement = $heroBlock.Trim() + "`r`n`r`n" + '  <main id="main" class="blog-page-main">'
  $blog = [regex]::Replace($blog, '<main id="main">', $mainReplacement, 1)
}
else {
  $blog = [regex]::Replace($blog, '<main id="main">', '<main id="main" class="blog-page-main">', 1)
}

$blog = [regex]::Replace(
  $blog,
  '<section class="blog-layout">\s*<div class="container" data-aos="fade-up">',
@'
...
$blog = [regex]::Replace(
  $blog,
  '<section class="tips-section">\s*<div class="container" data-aos="fade-up">',
@'
...
$blog = [regex]::Replace(
  $blog,
  '<section class="newsletter-section">\s*<div class="container" data-aos="fade-up">',
@'
...
$blog = [regex]::Replace(
  $blog,
  '<div class="col-lg-4">\s*<div class="sidebar-card">',
@'
...
if ($legacyCss -and $mainCss -notmatch 'Blog page custom styles moved from blog.html') {
  $mainCss = $mainCss.TrimEnd() + "`r`n`r`n/*--------------------------------------------------------------`r`n# Blog page custom styles moved from blog.html`r`n--------------------------------------------------------------*/`r`n" + $legacyCss + "`r`n"
}

if ($mainCss -notmatch 'Blog page styles aligned with index.html') {
  $mainCss = $mainCss.TrimEnd() + "`r`n`r`n" + $blogCss.Trim() + "`r`n"
}

Write-Text $BlogPath $blog
Write-Text $MainCssPath $mainCss
Write-Output 'Blog sync script updated successfully.'
