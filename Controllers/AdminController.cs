...
public class AdminController : Controller
{
    ...
    private async Task<AdminMarketingViewModel> BuildAdminMarketingViewModelAsync()
    {
        ...
        if (orderedImages.Count > 0 && !string.IsNullOrWhiteSpace(orderedImages[0].ImageUrl)) model.BannerImage1 = ResolveExistingBannerActiveImageUrl(orderedImages[0].ImageUrl!, 1);
        if (orderedImages.Count > 1 && !string.IsNullOrWhiteSpace(orderedImages[1].ImageUrl)) model.BannerImage2 = ResolveExistingBannerActiveImageUrl(orderedImages[1].ImageUrl!, 2);
        if (orderedImages.Count > 2 && !string.IsNullOrWhiteSpace(orderedImages[2].ImageUrl)) model.BannerImage3 = ResolveExistingBannerActiveImageUrl(orderedImages[2].ImageUrl!, 3);
    ...
    private async Task<string> NormalizeBannerActiveImageAsync(string? imageUrl, int orderIndex)
    {
        ...
        {
            return ResolveExistingBannerActiveImageUrl(imageUrl.Trim(), orderIndex);
        }
    ...
    private string ResolveExistingBannerActiveImageUrl(string imageUrl, int orderIndex)
    {
        if (string.IsNullOrWhiteSpace(imageUrl))
        {
            return string.Empty;
        ...
        var matchedFile = Directory.GetFiles(activeFolderPath, $"banner-{orderIndex}.*")
            .OrderBy(path => path)
            .FirstOrDefault();
        ...
        var extension = Path.GetExtension(matchedFile).ToLowerInvariant();
        return $"~/{BannerActiveFolderRelative}/banner-{orderIndex}{extension}";
    }
